/*
 * ElementalX msm_sleeper by flar2 <asegaert@gmail.com>
 * 
 *
...
static void msm_sleeper_early_suspend(struct early_suspend *h)
{
	...
	if (maxscroff) {
		...
		limit_set = true;
		pr_info("msm_sleeper: limit max frequency to: %d\n", maxscroff_freq);
	}
	return; 
}
...
static void msm_sleeper_late_resume(struct early_suspend *h)
{
	...
	limit_set = 0;
	pr_info("msm_sleeper: restore max frequency.\n");
	return;
}
...
{
	pr_info("msm_sleeper version %d.%d\n",
...
MODULE_DESCRIPTION("'msm_sleeper' - Limit max frequency while screen is off");
MODULE_LICENSE("GPL");
